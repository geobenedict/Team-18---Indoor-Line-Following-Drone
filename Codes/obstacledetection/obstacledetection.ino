...
int distance_limit = 30; // Distance limit in inches for obstacle detection
 
unsigned long distance_cm = 0;
int object_status = 1; //0= detected, 1= not detected
//Web Server initialisation on port 80
...
// Setup LED FLash if LED pin is defined in camera_pins.h
 ...
 // Start the camera server
  ...
  server.on("/readDistance", handleDistance);  // Serve the distance data
...
void loop() {
 ...
 // If the obstacle is detected within the limit, stop all actions
  if (distance_cm <= distance_limit) {
    ...
    Serial.println("Obstacle removed! Resuming operation...");
    object_status = 1; // No object detected
  }
 
  // Handle any incoming web requests
  server.handleClient();
 
  delay(1000); // Delay before checking the obstacle again
}
...
void readUltrasonicSensor() {
  ...
  // Convert the duration to distance in centimeters and inches
